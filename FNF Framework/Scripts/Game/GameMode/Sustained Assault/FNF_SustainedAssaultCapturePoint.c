...
class FNF_SustainedAssaultCapturePoint : SCR_CaptureArea
{
	...
	[Attribute("0 0 0", UIWidgets.EditBox, "Center of the objective in local space.", category: "Capture Point", params: "inf inf 0 purposeCoords spaceEntity")]
	...
	protected FNF_CoreNotificationManagerComponent m_Notifications;
	...
	protected override void OnInit(IEntity owner)
	{
		...
		m_Notifications = FNF_CoreNotificationManagerComponent.Cast(m_pGameMode.FindComponent(FNF_CoreNotificationManagerComponent));
		...
		if (m_CurrentOwnedPercentIncreasingRate != 0)
		{
			...
			else if (m_CurrentOwnedPercent >= 100)
			{
				...
				else
				{
					...
					m_CurrentOwnedPercentIncreasingRate = 0;
					FNF_NotificationObject notif = new FNF_NotificationObject(m_sPointName + " Captured", m_sPointName + " Captured by " + m_CurrentTimedFaction.GetFactionName(), 5.0);
					m_Notifications.AddNotification(notif, false);
				}
			}
		}
	}
